...
class WorkingHours extends Model
{
    ...
    {
        $rToday = new DateTime();
        $rResult = Database::getResultFromQuery(
            "SELECT name 
                FROM Users 
                WHERE end_date IS NULL 
                AND id NOT IN(
                    SELECT user_id FROM Working_hours 
                    WHERE work_date = '{$rToday->format('Y-m-d')}'
        ...
        $aAbsentUsers = [];
        if ($rResult->rowCount() != null) {
            while ($row = $rResult->fetchObject()) {
                array_push($aAbsentUsers, $row->name);
            }
        }
        return $aAbsentUsers;
    }

    public static function getTotalMonthlyWorkedTime($year, $month){
        $startDate = (new DateTime("$year-$month-01"))->format("Y-m-d");
        $lastDay = getLastDayOfMonthh($startDate)->format('d');
        $endDate = (new DateTime("$year-$month-$lastDay"))->format('Y-m-d');
